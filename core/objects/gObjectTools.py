...
from core.rendering.PyOGL import RenderObjectAnimated, RenderObjectStatic


# collision mask categories
COLLISION_CATEGORIES = ('obstacle', 'no_collision', 'mortal', 'character', 'player', 'light', 'trigger')
COLLISION_CATEGORIES = {x: 2 ** i for i, x in enumerate(COLLISION_CATEGORIES)}


# SHAPE FILTER
def add_ignore(filter_obj, categories):
...
def sub_ignore(filter_obj, categories):
    mask = filter_obj.mask
    if isinstance(categories, tuple):
        for x in categories:
            mask += COLLISION_CATEGORIES[x]
...
def shape_filter(categories, ignore=None, collide_with=None):
    __doc__ = """
    ...
    if not all([c in COLLISION_CATEGORIES for c in categories]):
        raise ValueError(f'Some of categories from: {categories}'
                         f' are not exist. Chose from {COLLISION_CATEGORIES.keys()}')
...
def rect_points(w, h, x_offset=0, y_offset=0):
    """
...
class Mortal:
    ...
    shape_filter = shape_filter(('mortal', ), )
